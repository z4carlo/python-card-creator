...
def BuildAttachProfile(AttachData, attachments_folder):
    print(f"Creating {AttachData['Name']} Profile")
    ...
    attach_portrait = Image.open(f"{attachments_folder}{AttachId}.jpg").convert('RGBA')
    if isCommander:
        attach_portrait = attach_portrait.resize(( 273, 283 ))
    else:
        attach_portrait = attach_portrait.resize(( 273, 345 ))
    canvas.add_layer(attach_portrait, 0, 0, depth=0)
    return canvas.render()
...
def main():
    ...
    if not os.path.exists(AttachCardsOutputDir+"png/"):
        Path(AttachCardsOutputDir+"png/").mkdir(parents=True, exist_ok=True)
    if not os.path.exists(AttachCardsOutputDir+"jpg/"):
        Path(AttachCardsOutputDir+"jpg/").mkdir(parents=True, exist_ok=True)
    ...
    for SelectedAttachmentCardData in AsoiafData['attachments']:
        ...
        attach_card_output_path = os.path.join(AttachCardsOutputDir+"png/", f"{SelectedAttachmentCardData['Id'].replace(' ', '_')}f.png")
        attach_card.save(attach_card_output_path)
        attach_card_output_path = os.path.join(AttachCardsOutputDir+"jpg/", f"{SelectedAttachmentCardData['Id'].replace(' ', '_')}f.jpg")
        attach_card = attach_card.convert("RGB")
        attach_card.save(attach_card_output_path)
    ...
    for SelectedAttachmentCardData in AsoiafData['attachments']:
        ...
        if not is_any_value_true:
            continue
        attach_card = BuildAttachProfile(SelectedAttachmentCardData, attachments_folder)
        attach_card_output_path = os.path.join(AttachCardsOutputDir+"png/", f"{SelectedAttachmentCardData['Id'].replace(' ', '_')}p.png")
        attach_card.save(attach_card_output_path)
        attach_card_output_path = os.path.join(AttachCardsOutputDir+"jpg/", f"{SelectedAttachmentCardData['Id'].replace(' ', '_')}p.jpg")
        attach_card = attach_card.convert("RGB")
        attach_card.save(attach_card_output_path)
